...
import { type AdminContext, requireAdmin, requireOwner } from "../../middleware/rbac";
    ...
    `);
		...
		const normalize = (day: any) => {
			...
			if (Array.isArray(day.intervals)) {
				return {
					closed: !!day.closed,
					intervals: day.intervals.map((i: any) => ({ open: i.open, close: i.close })),
				};
			}
    ...
    `);
		...
		const zones =
			result.rows.length > 0
				? result.rows[0].value
				: [
						{ name: "Zone 1", radius: 2, fee: 2.99, minimum_order: 15.0 },
						{ name: "Zone 2", radius: 5, fee: 4.99, minimum_order: 20.0 },
					];

...
inventory.patch("/:id/stock", requireAdmin, async (c: AdminContext) => {
  try {
    ...
    if (newStock > currentItem.current_stock) {
      updateFields.push(`last_restocked_at = $${paramIndex}`, `last_restocked_by = $${paramIndex + 1}`);
  ...
  try {
    ...
    try {
      ...
      for (const update of updates) {
        ...
        // Get current stock
        const current = await client.query('SELECT current_stock FROM inventory WHERE id = $1', [inventoryId]);

...
describe('Menu API Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    for (const item of menuItems) {
      ...
      `, [item.name, item.description, item.price, item.category, item.image, item.allergies, item.is_available])
    }
  })
  ...
  describe('GET /menu', () => {
    it('should return all available menu items', async () => {
      ...
      const unavailableItem = response.body.items.find((item: any) => item.name === 'Unavailable Item')
  ...
  describe('GET /menu/item/:id', () => {
    ...
    it('should return 404 for unavailable item', async () => {
      ...
      const response = await request(app.fetch)
        .get(`/menu/item/${unavailableItemId}`)
        .expect(404)
